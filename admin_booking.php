...
if (isset($_GET['action']) && isset($_GET['booking_id'])) {
    $action = $_GET['action'];
    $booking_id = intval($_GET['booking_id']); // Ensure the ID is an integer
    ...
    if ($conn->query($updateQuery) === TRUE) {
        header("Location: admin_booking.php"); // Refresh the page after the action
        exit();
    } else {
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <a href="?action=approve&booking_id=<?php echo $row['booking_id']; ?>" class="btn btn-success">Approve</a>
                                <a href="?action=reject&booking_id=<?php echo $row['booking_id']; ?>" class="btn btn-danger">Reject</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
